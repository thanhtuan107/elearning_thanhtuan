...
import { I_PropLogged } from "../../../interfaces/userManagementInterface";
...
import { setIsOpenModalAuthREDU } from "../../../store/slices/modalSlice";
import { LoadingOutlined } from "@ant-design/icons";
import { setIsLoadingBtnREDU, setIsLoadingPageREDU } from "../../../store/slices/loadingSlice";
import { wait } from "../../../helpers/awaitHelper";
import { DELAY_LOADING_PAGE } from "../../../constants/configContants";
...
function DetailCoursePage() {
    ...
    useEffect(() => {
        const fetch = async () => {
            if (id !== undefined) {
                try {
                    ...
                    const { data, status } = await courseApi.getCourseById(id); // Fetch course details

                    console.log("fetch - getCourseById", { data, status });

                    setCourse(data.result.data); // Set course details
        ...
        fetch();
    }, [id]); // Fetch course details when the route parameter changes
    ...
    const renderHoctiep = () => {
        ...
        const flag = (userLogin as any).enrolledCourseDetail.findIndex((course: { _id: string }) => course._id === id);
