...
function Button({ name, htmlFor, children, type, className, onClick, disabled = false }: Props) {
    ...
    let classDisabled = "";
    if (type === "primary") {
        classType = "rounded-lg border-2 border-black bg-white text-black font-bold px-5 py-2 hover:text-white hover:bg-black hover:border-black/50 active:bg-black/80 active:text-white text-sm";
    }
    if (type === "transparent") {
